...
不合格なら、帰属部分だけを訂正して最大3回再生成し、各候補を再検証する。ただし一連の
検証は30秒で打ち切る。上限までに合格しなければ、誤った帰属を含む下書きを表示せず、
`validation_failed`状態と中立なフォールバックを返す。
...
**実装状態: 完了（明示日付と入力制約の第一段階）。** `POST /temporal/validate`と複合候補gateを
実装した。候補だけでなく任意の`request_content`も受け取り、ユーザー入力中の相対日付と
絶対日付の矛盾を応答候補と照合する。曖昧な相対表現のSLM構造化は、決定論的検査で
取り出せない表現へ段階的に拡張する。
...
照合する。また、日付を含む文と、その直後にある通知・リマインドの約束を一つの時間主張
として扱う。入力に「明日」と過去の絶対日付が併記されている場合は、応答が矛盾を明示して
確認または拒否しない限り通過させない。回答全体の事実性は検証しない。claimは次のような
構造へ正規化する。
...
主体帰属gateと同様、矛盾候補を除外し、最大3回または30秒まで限定再生成して再検証する。
両gateを通す場合は、抽出済みclaimと根拠解決結果を共有し、同じ候補を何度もSLMへ送らない。
上限超過時は`validation_failed`を明示し、棄却候補本文と失敗表示を記憶へ取り込まない。
再生成には棄却文全文を再提示せず、構造化した違反理由だけを渡す。これは軽量モデルが
誤った日付や帰属表現へ固定され、同じ内容を言い換えて再提出することを避けるためである。

編集可能・分岐可能なフロントエンドでは、画面上のmessage IDと内容digestから不変event IDを
作る。同じ内容の再送は冪等に扱い、編集後の内容は旧eventの上書きではなく別versionとする。
また、生成直後のassistant兄弟応答をすべて取り込まず、次のuser turnへ実際につながった
active path上の応答だけを補助文脈として取り込む。

**Stage D受け入れ基準**

- 現在日より過去の期限を未来のリマインドとして提示する候補を拒否する
- 日付と実行約束を複数文へ分離しても、隣接文として同じ制約を適用する
- 入力自体の相対日付と絶対日付が矛盾する場合、黙って一方を採用する候補を拒否する
- 棄却文を再生成promptへ再注入せず、構造化feedbackだけで再生成できる
- 編集発話と手動再生成branchが、同一eventの上書きや全枝の補助文脈化を起こさない
...
#### 16.6.1 会話境界とコンテキスト予算

**実装状態: 完了。** 夜間処理は全日ログを一つのpromptへ平坦に連結せず、次の階層へ分割する。

1. 同一conversation内でも、user発話間隔が設定値を超えた位置をsession境界とする
2. session内の明示的な転換表現をtopic境界候補とする
3. SLMまたは夜間LLMが前後の発話を比較し、実際のtopic転換だけを構造化する
4. topic区間を推定token予算でchunk化し、境界前後の少数turnを重複文脈として含める
5. chunk単位で候補抽出した後、source event IDを決定論的に照合する

session境界とtopic境界は同一概念ではない。前者は時間的なエピソード区切りであり、後者は
同一の対話時間帯に含まれる意味的区間である。DBでは`conversation_segments`の親子関係として
別々に保持し、生の`raw_messages`は変更しない。境界は観測事実ではなく派生推定なので、
`detection_source`、`confidence`、`signals`、使用モデルを記録する。

明示的な転換語は候補生成にのみ使う。同じ表現が要約や話題継続にも使われるため、LLMが
利用可能な場合は辞書一致だけでtopic境界を確定しない。LLMが停止した場合に限り、ルール候補を
低い確信度のfallbackとして利用できる。

夜間候補抽出にはassistant／system発話も文脈として渡すが、候補の根拠として許可するevent IDは
未処理のuser発話に限定する。重複文脈は理解の連続性のために使い、一次所属eventはchunk間で
重複させない。候補は要約指紋、memory type、source ID集合で重複排除する。

既定値はsession空白90分、context予算12,000推定token、前後2turn重複、topic境界閾値0.68とする。
これらは固定仕様ではなく初期評価値である。`batch_jobs.result_json`へ、境界数、区間数、推定入力
token、境界判定と抽出のモデル呼び出し回数、失敗chunkを保存し、公開ログを必要とせず比較できる。

リアルタイムclaim抽出モデルと夜間モデルは分離する。`HIPPOCAMPUS_NIGHTLY_MODEL`または実行時の
`model`指定を優先し、未指定時だけ主対話用モデルを利用する。長いコンテキストは生ログの一括処理
ではなく、topic抽出後の日次横断評価や区間間の重複・訂正統合へ使う。
